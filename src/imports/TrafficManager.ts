    ...
    for (const coord of [...emptyTiles, ...occupiedTiles]) {
        ...
        if (!visitedCreeps.has(occupyingCreep.name)) {
            if (getIntendedPackedCoord(occupyingCreep) === packedCoord)
                score -= getMovePriority(occupyingCreep as CreepType);
...
function resolveMovement(creep: AnyCreep): void {
 ...
 */
function canMove(creep: AnyCreep): boolean {
...
function getIntendedPackedCoord(creep: AnyCreep): number | undefined {
    return (creep as any)._intendedPackedCoord;
}

function getMatchedPackedCoord(creep: AnyCreep): number | undefined {
    return (creep as any)._matchedPackedCoord;
}

function deleteMatchedPackedCoord(creep: AnyCreep): void {
