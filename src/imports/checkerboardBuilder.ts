import { generateName } from "functions/misc"
...
export function createAndUpdateCheckerSite(memBase: { [link: string]: any }, memLink: string, spawnname: string) {
    ...
    //try {
        ...
        if (memBase[memLink].confirmed === false) {
            console.log(test.checkerCalculate(spawnname))
            console.log(memBase+" "+memLink)
        ...
        //if((Game.spawns[spawnname].room.controller??{level:0}).level>=6) {
        //    if (global.isextractor===undefined) {
        //        let minloc = Game.spawns[spawnname].room.find(FIND_MINERALS)[0].pos
        //        let extractor = minloc.createConstructionSite(STRUCTURE_EXTRACTOR)
        //    }
        //}
...
export class checkerBoard {
    ...
    buildChecker = function (radius: number, mid: { x: number, y: number }, spawn: {room:Room,pos:RoomPosition}) {
        ...
        let newopen = [{ x: mid.x, y: mid.y - 1 }, { x: mid.x + 1, y: mid.y - 2 }, { x: mid.x, y: mid.y - 2 }, { x: mid.x - 1, y: mid.y - 2 }, { x: mid.x, y: mid.y - 3 }]
        //spawn.room.visual.circle(new RoomPosition(mid.x, mid.y, spawn.room.name), { stroke: "yellow" })
        for (let fail of newopen) {
