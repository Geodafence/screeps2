...
export class checkerBoard {
    ...
    checkerCalculate(spawnName:string) {
        let spawn = Game.spawns[spawnName]
        var infoPak = this.data
        let RCL = infoPak.RCL
    ...
    buildChecker = function(radius:number,mid:{x:number,y:number},spawn:StructureSpawn) {
        let builds = []
        ...
        for(let road of dirs) {
            ...
            for(let wall of roads) {
                if(spawn.room.getTerrain().get(wall.x,wall.y)===1) {
        ...
        for(let fail of [{x:mid.x,y:mid.y-1},{x:mid.x+1,y:mid.y-2},{x:mid.x,y:mid.y-2},{x:mid.x-1,y:mid.y-2},{x:mid.x,y:mid.y-3}]) {
            if(spawn.room.getTerrain().get(fail.x,fail.y)===1) {
